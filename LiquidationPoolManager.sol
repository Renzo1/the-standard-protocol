...
contract LiquidationPoolManager is Ownable {
    ...
    function distributeFees() public {
        // @q how does this eurosToken get to this contract. Ans: It doesn't - vuln
        // Show the map of value to prove this. convert png/jpeg to ASCII for report
    ...
    // @audit Potential DoS attack leading to bad debt
    // Attack: DoS attack distributeFees()
    // Causing this function not to run
    // Thus vault not to be liquidated.
    // @q what more damages can I cause with an undercollateralized vault? lol
    function runLiquidation(uint256 _tokenId) external {
        ISmartVaultManager manager = ISmartVaultManager(smartVaultManager);
        manager.liquidateVault(_tokenId); // liquidate vault and send collateral to protocol (treasury)
        distributeFees(); // @audit Potential DoS. cause to OOG error
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            // @audit why is ILiquidationPoolManager.Asset.amount filled with this balance?
            // I expect this balance to be zero for all tokens
